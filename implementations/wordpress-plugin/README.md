...
|---|---|
| **Registry** ([04](../../spec/04-registry-contract.md)) | the **collector** — receives `agentify_discovery_register`, exposes `register()` / `add()` and `add_well_known()`, validates synchronously, auto-attributes `provider` via backtrace |
...
- The `agentify_discovery_register` action and the `WP_Discovery` facade.
...
No dependency guard is needed for the hook form: if Agentify is not installed, `agentify_discovery_register` never fires. For the direct facade form, guard with `class_exists('Agentify_Discovery')`. See [04-registry-contract.md](../../spec/04-registry-contract.md).
